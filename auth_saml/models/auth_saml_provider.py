...
class AuthSamlProvider(models.Model):
    ...
    def _validate_auth_response(self, token: str, base_url: str = None):
        ...
        matching_value = None
        response_data = xmltodict.parse(str(response.response))
        json_dict = json.loads(str(json.dumps(response_data)))
        if self.matching_attribute == "subject.nameId":
            #matching_value = response.name_id.text
            matching_value = json_dict["ns0:Response"]["ns1:Assertion"]["ns1:AttributeStatement"]["ns1:Attribute"][1]["ns1:AttributeValue"]["#text"]
        else:
