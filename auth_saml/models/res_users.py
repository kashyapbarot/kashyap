...
class ResUser(models.Model):
    ...
    @api.model
    def _auto_update_saml_details(self):
        """
        This function will use to update SAML details.
        Created by: @ashvin, @bistasolutions
        """
        for rec in self.search([('saml_ids', '=', False)]):
            providers = self.env["auth.saml.provider"].search([])
            for provider in providers:
                rec.write({'saml_ids': [
                    (0, 0, {'saml_provider_id': provider.id,
                            'saml_uid': rec.login})]})
