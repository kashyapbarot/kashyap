...
class SAMLLogin(Home):
    ...
    def web_login(self, *args, **kw):
        ...
        # for not allowed odoo users login and password if we have not enabled
        # start code @ashvin, @bistasolutions
        if not request.env["ir.config_parameter"].sudo().get_param("auth_saml.allow_saml_uid_and_internal_password"):
            values = request.params.copy()
            try:
                values['databases'] = http.db_list()
            except odoo.exceptions.AccessDenied:
                values['databases'] = None
            values['error'] = _("System do not allow direct login.\n"
                                "Please login with Okta.")
            response = request.render('web.login', values)
            response.headers['X-Frame-Options'] = 'DENY'
            response.qcontext["providers"] = providers
            return response
        # end code @ashvin @bistasolution
...
class AuthSAMLController(http.Controller):
    ...
    def signin(self, req, **kw):
        ...
        # updated condition @ashvin @bistasolution
        if kw.get("RelayState") in (None, ''):
        ...
        # updated condition @ashvin @bistasolution
        state = json.loads(kw["RelayState"]) if kw["RelayState"] else None
        provider = state["p"]
        dbname = state["d"]
        context = state.get("c", {})
